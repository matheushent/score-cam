...
class ScoreCAM:
    ...
    def generate_cam(weights, maps):
        ...
        Inputs are the weights (shape Kx1x1x1) generated by the foward computing F(Mk)
        followed by softmax activation and normalized maps (shape KxHxWx3)

        Args:
            weights (numpy.ndarray): Output score with shape (K, 1, 1, 1) where
            K is the number of filters in the last convolutional layer
            maps (tf.Tensor): 4D-Tensor with shape (K, H, W, 1) where K is the number
